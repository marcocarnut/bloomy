 ...
 * for completeness.
 */
/* --------- reseed39 patch: minimal static file serving for non-WS GET --------- *
 * wsServer is WS-only; when a request isn't a WS upgrade, do_handshake used to just
 * close. Instead, if a www root is set (ws_set_www), serve a file so ONE process on ONE
 * port serves both the reseed39 bundle (GET /) and the bloom WS. HTTP/1.0-style
